...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Pega a primeira empresa para associar (ajuste conforme sua lógica de multi-empresa)
        empresa = Empresa.objects.first()
        
        if not empresa:
            self.stdout.write(self.style.ERROR('Nenhuma empresa encontrada. Cadastre uma empresa antes.'))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f'{count} exames foram cadastrados com sucesso para a empresa {empresa.nome_fantasia}!'))